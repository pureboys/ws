...
pub async fn get_course_detail_db(pool: &PgPool, teacher_id: i32, course_id: i32) -> Course {
    let row = sqlx::query!(
        r#"select id,teacher_id,name,time from course where teacher_id = $1 and id = $2"#, teacher_id, course_id
...
pub async fn post_new_course_db(pool: &PgPool, new_course: Course) -> Course {
    let row = sqlx::query!(
        ...
        new_course.id.unwrap(), new_course.teacher_id, new_course.name
    ).fetch_one(pool).await.unwrap();
